...
<div id="<?php echo $graphName; ?>"><!-- Plotly chart will be drawn inside this DIV --></div>
  <script>
    ...
    while($row !== false)
    {
        ...
        if($interval < DAILY)
        {
            if($endTime-$startTime > DAILY) //Hourly over multiple days
            {
        ...
        if($interval < DAILY) //daily formats for night/morning/day/evening colors
        {
            $hour=date('H',$prevRow['timestamp']);
            if($hour == 0) //midnight
            {
        ...
        else //default color for non-hourly charts
        {
